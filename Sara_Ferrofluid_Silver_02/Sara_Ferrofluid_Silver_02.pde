...
void setup() {
  ...
  style = new int[cols*rows*2]; // color, render style
  setupStyle();
}

// Where the Magic Happens
void draw() {
  background(0); // Paint the background black at the start of every frame.
  ...
  int counter =0; // to navigate around the style colors
  ...
  for (int y = 0; y < rows - 1; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      //fill(style[counter] );  // Dark red color for h1;
      vertex(modifiedX1, modifiedY1, h1);

      //fill(map(h2, -50, 0, 50, 100), map(h2, -50, 0, 50, 100), map(h2, -50, 0, 60, 120));  // Gunmetal silver color.
